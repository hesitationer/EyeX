...
void
EyeT::setType(TransformType type)
{
	this->type = type;
	switch (type)
	{
		case TRANSFORM_IDE:
			this->app = cv::warpAffine;
			this->get = getTransformIde;
			break;
		case TRANSFORM_TRA:
			this->app = cv::warpAffine;
			this->get = getTransformTra;
			break;
		case TRANSFORM_SIM:
			this->app = cv::warpAffine;
			this->get = getTransformSim;
			break;
		case TRANSFORM_AFF:
			this->app = cv::warpAffine;
			this->get = getTransformAff;
			break;
		case TRANSFORM_PER:
			this->app = cv::warpPerspective;
			this->get = getTransformPer;
			break;
		default:
			this->type = TRANSFORM_PER;
			this->app = cv::warpPerspective;
			this->get = getTransformPer;
			break;
	}
...
cv::Mat
EyeT::getTransform(cv::Point2f *pt_src, cv::Point2f *pt_dst, int pt_count) {
	cv::Mat op = this->get(pt_src, pt_dst, pt_count);
	
	return op;
}

void
EyeT::appTransform(cv::InputArray src, cv::OutputArray dst, cv::InputArray op) {
	this->app(src, dst, op, src.size(), cv::INTER_LINEAR, cv::BORDER_CONSTANT, cv::Scalar());
}
...
cv::Mat
EyeT::getTransformIde(
					  const cv::Point2f pt_src[], const cv::Point2f pt_dst[], int pt_count)
{
	cv::Point2f s0 = pt_src[0];
	cv::Point2f d0 = pt_dst[0];
...
cv::Mat
EyeT::getTransformSim(
					  const cv::Point2f pt_src[], const cv::Point2f pt_dst[], int pt_count)
{
	cv::Point2f s = pt_src[1] - pt_src[0];
	cv::Point2f d = pt_dst[1] - pt_dst[0];
	double s_norm = norm(s);
	double d_norm = norm(d);
	double alpha = d_norm / s_norm;
	
	double cs = (cv::Mat(s).dot(cv::Mat(d)))/(s_norm*d_norm);
	...
	cv::Point2f b = d0 - cv::Point2f((float)(+cs*s0.x +sn*s0.y), (float)(-sn*s0.x +cs*s0.y));
...
cv::Mat
EyeT::getTransformPer(
					  const cv::Point2f pt_src[], const cv::Point2f pt_dst[], int pt_count)
{
	int l = pt_count*2;
	cv::Mat M(3, 3, CV_64F), X(8, 1, CV_64F, M.data);
	cv::Mat A(l, 8, CV_64F), B(l, 1, CV_64F);
	...
	solve( A.t()*A, A.t()*B, X, cv::DECOMP_SVD );
	(( double*)M.data)[8] = 1.;
